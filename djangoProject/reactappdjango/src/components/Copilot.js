...
const StyledShape = styled.div`
    width: ${(props) => (props.isSquare ? '450px' : '70px')};
    height: ${(props) => (props.isSquare ? '650px' : '70px')};
    border-radius: ${(props) => (props.isSquare ? '5%' : '50%')};
    background-color: ${(props) => (props.isSquare ? 'lightgray' : 'gray')};
    cursor: pointer;
    position: fixed;
    bottom: ${(props) => (props.isSquare ? '20px' : '40px')};
    right: ${(props) => (props.isSquare ? '20px' : '40px')};

    transition: all 0.3s ease; 

    &:hover {
    width: ${(props) => (props.isSquare ? '450px' : '90px')};
    height: ${(props) => (props.isSquare ? '650px' : '90px')};
    }
    `;
...
export default function Copilot({ editorContent, idFromLanding }) {
    ...
    const fetchOne = async () => {
        try {
            const response = await axios.get(`http://localhost:8000/logs/one?id=${idFromLanding}`);
    ...
    const fetchLogs = async () => {
        ...
        axios
            .get('http://localhost:8000/logs/all')
            .then(response => {
    ...
    const handleUpdate = (new_content) => {
        ...
        axios
            .put('http://localhost:8000/logs/update', {
                id: idFromLanding,
    ...
    const handleLogCreate = (logToStore, task, name) => {
        ...
        axios
            .post('http://localhost:8000/logs/create', {
    ...
    const handleLogDelete = (idToDel) => {
        // Send POST request to 'books/create' endpoint
        axios
            .put('http://localhost:8000/logs/delete', {
                id: idToDel,
            })
            .then(res => {
    ...
    const handleLogDeleteAll = () => {
        ...
        axios.put('http://localhost:8000/logs/deleteAll')
            .then(() => {
                // Fetch all books to refresh
                // the books on the bookshelf list
                fetchLogs()
    ...
    const sendMessage = (message, flag) => {
        // TODO divert message to django, perform computations
        console.log("MESSAGE = " + JSON.stringify(message));
        const url = "https://api.openai.com/v1/chat/completions"; // old openai url
        // const url = "/api/receive_data/"; // url to django
        const apiKey = "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"; // Your OpenAI API key
        const data = {
        ...
        axios
            ...
            .then((response) => {
                if (!flag) {
                    ...
                    // conversationRef.current.push({ role: "assistant", content: response.data.message });
                    conversationPersistent = conversationRef.current;
                    const r = "assistant";
                    const q = response.data.choices[0].message.content;
                    // const q = response.data.message;
                    const newChat = { type: "bot", message: response.data.choices[0].message.content };
                    // const newChat = { type: "bot", message: response.data.message };
    ...
    const handleClick = () => {
        setIsSquare((prevIsSquare) => !prevIsSquare);
    };
    ...
    return (
        <StyledShape isSquare={isSquare} onClick={isSquare ? null : handleClick}>
            {isSquare && <div className="container mx-auto max-w-[700px]">
                <div className="flex flex-col h-screen bg-gray-900" style={{ maxHeight: "85vh", backgroundColor: "#e9e9e9" }}>
                    <button style={{color: 'white', background: 'black', margin: '0 0 0 430px'}} onClick={handleClick}>
                        X
                    </button>
                    <img
                        src={harvest_logo}
